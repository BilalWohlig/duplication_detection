...
const pushDataToPineconeV2 = async (req, res) => {
  try {
    let data = await DummyData.find()
    data = data.map((d) => d.userData)
    const result = await GoogleService.pushDataToPineconeV2(data, req.body.flag)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('pushDataToPineconeV2 Error', err)
    return res.sendJson({
...
router.post('/pushDataToPineconeV2', validation, pushDataToPineconeV2)
