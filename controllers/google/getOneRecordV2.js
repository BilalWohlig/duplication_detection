...
const getOneRecordV2 = async (req, res) => {
  try {
    const result = await GoogleService.getOneRecordV2(req.params.id)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('getOneRecordV2 Error', err)
    return res.sendJson({
...
router.get('/getOneRecordV2/:id', validation, getOneRecordV2)
