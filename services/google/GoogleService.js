...
class GoogleService {
  ...
  async pushSingleDataToDb(data) {
    const newRecord = new DummyData({
      userData: data
    })
  ...
  async updateAllDocuemntsInDB() {
    // let docs = 0
    const records = await DummyData.find().sort({_id:-1}).limit(35)
    // console.log(records.length)
    for (let i = records.length - 15; i >= 0; i--) {
      const record = records[i];
      if(!record.pineconeId) {
        return 'Shutup'
        const document = await index.query({
          ...
          filter: {
            ...
            "MOTHER_NAME_ENG": {"$eq": record.userData.MOTHER_NAME_ENG},
            "DOB": {"$eq": record.userData.DOB},
            "ALL_DATA": {"$eq": Object.values(record.userData).join(", ")}
          }
        })
        ...
        // return matchingDocInPinecone
        for (let j = 1; j < matchingDocInPinecone.matches.length; j++) {
          const doc = matchingDocInPinecone.matches[j];
          if(Math.round(doc.score) >= 0.9) {
            let matchedDoc
            const matchingDocsInDB = await DummyData.find({"userData.FAMILYIDNO": doc.metadata.FAMILYIDNO, "userData.FATHER_NAME_ENG": doc.metadata.FATHER_NAME_ENG, "userData.HOF_NAME_ENG": doc.metadata.HOF_NAME_ENG, "userData.DOB":  doc.metadata.DOB, "userData.MOTHER_NAME_ENG": doc.metadata.MOTHER_NAME_ENG})
            for (let k = 0; k < matchingDocsInDB.length; k++) {
              const rec = matchingDocsInDB[k];
              const stringData = Object.values(rec.userData).join(", ")
              if(stringData == doc.metadata.ALL_DATA) {
                matchedDoc = rec
                break
              }
            }
            if(matchedDoc) {
              matchedDoc.pineconeId = doc.id
              matchedDoc.mostSimilarDocument = record._id,
              matchedDoc.similarityScore = Math.round(doc.score * 100)
        ...
        await record.save()
        return `Record Updated ${i}`
      }
      else{
        console.log("Already Updated", i)
      }
      // return "Heyyyyy"
  ...
  async getRecordsAboveThreshold(threshold, page) {
    ...
    return await DummyData.find({similarityScore: {$gte: threshold}, status: {$exists: false}}).skip(skip).limit(limit)
  }
